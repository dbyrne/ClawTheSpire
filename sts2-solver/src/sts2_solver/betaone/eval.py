...
def _scenario_to_combat_state_json(sc: Scenario) -> str:
    """Convert an eval Scenario to a Rust-compatible CombatState JSON."""
    p = sc.player
    powers = p.get("powers", {})

    # Build Card dicts matching Rust's Card struct
    hand_cards = []
    for card in (sc.hand or []):
        c = {
            "id": card.get("id", "UNKNOWN"),
            "name": card.get("name", "Unknown"),
            "cost": card.get("cost", 1),
            "card_type": card.get("card_type") or "Attack",
            "target": card.get("target") if card.get("target") not in (None, "None") else "Self",
            "upgraded": card.get("upgraded", False),
            "damage": card.get("damage"),
            "block": card.get("block"),
            "hit_count": card.get("hit_count", 1),
            "powers_applied": card.get("powers_applied", []),
            "cards_draw": card.get("cards_draw", 0),
            "energy_gain": card.get("energy_gain", 0),
            "hp_loss": card.get("hp_loss", 0),
            "keywords": list(card.get("keywords", {}).keys()) if isinstance(card.get("keywords"), dict) else list(card.get("keywords", [])),
            "tags": list(card.get("tags", {}).keys()) if isinstance(card.get("tags"), dict) else list(card.get("tags", [])),
            "spawns_cards": card.get("spawns_cards", []),
        }
        hand_cards.append(c)
    ...
    state = {
        "player": {
            "hp": p.get("hp", 70),
            "max_hp": p.get("max_hp", 70),
            "block": p.get("block", 0),
            "energy": p.get("energy", 3),
            "max_energy": p.get("max_energy", 3),
            "powers": powers,
            "hand": hand_cards,
            "draw_pile": hand_cards[:3] if hand_cards else [],  # some cards for MCTS to simulate draws
            "discard_pile": [],
            "exhaust_pile": [],
            "potions": [],
        },
...
def run_eval_mcts(
    ...
    """Run eval scenarios using MCTS search instead of greedy policy."""
    import sts2_engine

    # Need ONNX model — load network and export
    card_vocab = _load_card_vocab()
    num_cards = len(card_vocab)
    net = BetaOneNetwork(num_cards=num_cards)
    ...
    onnx_path = export_onnx(net, "betaone_checkpoints/onnx")
    ...
    for sc in scenarios:
        state_json = _scenario_to_combat_state_json(sc)

        # Run MCTS search
        mcts_result = sts2_engine.betaone_mcts_search(
        ...
        mcts_chosen_idx = mcts_result["chosen_idx"]
        policy = mcts_result["policy"]

        # MCTS enumerates actions from the CombatState, which may differ from
        # the scenario's hand-crafted action list (deduplication, ordering).
        # We need to use the same action enumeration to map back.
        # Run enumerate_actions on the same state to get the MCTS action list,
        # then match the chosen MCTS action to scenario actions by semantics.

        # Build scenario action descriptors for matching
        n_actions = len(sc.actions)
        hand = sc.hand or []

        # Match MCTS chosen action to scenario action by card index/type
        # MCTS actions are ChooseCard{choice_idx} for pending choices,
        # PlayCard{card_idx, target_idx} for plays, EndTurn, etc.
        # We match by: for ChooseCard, the card at choice_idx in hand;
        # for PlayCard, the card at card_idx in hand + target.
        # Since we can't see the MCTS action directly from Python, use the
        # policy distribution: map each MCTS action to a scenario action.

        # Simpler approach: the MCTS policy has one entry per MCTS-enumerated action.
        # For pending choices, MCTS produces ChooseCard{0}, ChooseCard{1}, etc.
        # Scenario also lists actions as choose_card with specific cards.
        # If the CombatState hand matches the scenario hand, indices align for
        # ChooseCard actions. For PlayCard, MCTS deduplicates but scenario may not.

        # For now: directly use mcts_chosen_idx against scenario actions.
        # This works when hand order and action enumeration match.
        chosen_idx = min(mcts_chosen_idx, n_actions - 1)

        # Build prob dict from MCTS policy mapped to scenario actions
        probs = {}
        for i in range(n_actions):
            p = policy[i] if i < len(policy) else 0.0
            probs[str(sc.actions[i])] = round(p, 3)
        ...
        result = {
            ...
            "chosen": str(sc.actions[chosen_idx]),
            "chosen_idx": chosen_idx,
            "chosen_prob": policy[chosen_idx] if chosen_idx < len(policy) else 0.0,
            "probs": probs,
        }
    ...
    # Print report
    print(f"\n{'='*60}")
    print(f"BetaOne MCTS Eval — Gen {gen} — {total_pass}/{total_pass + total_fail} passed (sims={num_sims})")
    print(f"{'='*60}\n")
...
def main():
    ...
    parser.add_argument("--mcts", type=int, default=0,
                        help="Run with MCTS search (specify num sims, e.g. --mcts 100)")
    args = parser.parse_args()
    if args.mcts > 0:
        run_eval_mcts(args.checkpoint, num_sims=args.mcts)
    else:
