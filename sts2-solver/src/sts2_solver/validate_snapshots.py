...
def simulate_turn(
    ...
    # Exception: block from Defend or Attack+self_block persists into next snapshot.
    for enemy in s.enemies:
        has_granted_block = (
            enemy.intent_type == "Defend"
            or getattr(enemy, "_has_move_table_block", False)
        )
        if not has_granted_block:
            enemy.block = 0
